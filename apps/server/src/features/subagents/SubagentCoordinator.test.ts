...
describe("SubagentCoordinator", () => {
  ...
  it("keeps complete settled turn output", () => {
    const output = Array.from({ length: 700 }, (_, index) => `${index}: ${"x".repeat(40)}`).join(
      "\n",
    );
    const thread = {
      messages: [{ role: "assistant", turnId: "turn-1", text: output }],
    } as unknown as OrchestrationThread;

    expect(output.length).toBeGreaterThan(24 * 1_024);
    expect(__testing.outputForTurn(thread, "turn-1")).toBe(output);
  });
