...
const WAIT_CHILD_LIMIT = 16 * 1_024;
const WAIT_TOTAL_LIMIT = 48 * 1_024;
const DELIVERY_LIMIT = 24 * 1_024;
const DELIVERY_LINE_LIMIT = 600;
...
function truncateText(text: string, maxChars: number, maxLines = Number.POSITIVE_INFINITY) {
  const lines = text.split("\n");
  let value = lines.slice(0, maxLines).join("\n");
  const truncated = lines.length > maxLines || value.length > maxChars;
  const notice = "\n\n[truncated; open the child thread for full output]";
  if (truncated) {
    value = `${value.slice(0, Math.max(0, maxChars - notice.length))}${notice}`.slice(0, maxChars);
  }
  return {
    text: value,
    truncated,
...
const make = Effect.gen(function* () {
  ...
  const findChildren = Effect.fn("SubagentCoordinator.findChildren")(function* (
    ...
    const selected = displayIds.map((id) => byId.get(id));
    const missing = displayIds.filter((_, index) => !selected[index]);
    if (missing.length > 0)
      return yield* fail("ids", `Unknown subagent IDs: ${missing.join(", ")}.`);
    return selected as OrchestrationThread[];
  });
      ...
      return yield* Effect.acquireUseRelease(
        ...
        () =>
          Effect.gen(function* () {
            ...
            let remainingBudget = WAIT_TOTAL_LIMIT;
            for (const initialChild of initial) {
              ...
              const budget = Math.min(WAIT_CHILD_LIMIT, remainingBudget);
              const output =
                result.state === "delivered"
                  ? {
                      text: "[result was already delivered to the parent thread]",
                      truncated: false,
                    }
                  : truncateText(outputForTurn(child, result.childTurnId), budget);
              remainingBudget = Math.max(0, remainingBudget - output.text.length);
              sections.push({
                displayId: relation.displayId,
                threadId: child.id,
                outcome: result.outcome,
  ...
  const check: SubagentCoordinatorShape["check"] = Effect.fn("SubagentCoordinator.check")(
    function* (parentThreadId, displayId) {
      const [child] = yield* findChildren(parentThreadId, [displayId]);
      yield* settle(child!);
      const refreshed = (yield* getThread(child!.id)) ?? child!;
