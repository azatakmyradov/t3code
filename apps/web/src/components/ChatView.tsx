...
function ChatViewContent(props: ChatViewProps) {
  ...
  const subagentsIntegration = useSubagentsIntegration(
    environmentId,
  ...
  const subagentsSummaryInset = subagentsIntegration.available ? 32 : 0;
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <AgentsPanel
        ...
        nativeSummaries={subagentsIntegration.nativeSummaries}
        nativePending={subagentsIntegration.nativePending}
        nativeError={subagentsIntegration.nativeError}
        parentThreadId={serverThread?.id ?? null}
        parentUpdatedAt={serverThread?.updatedAt ?? null}
        parentSessionActive={
          serverThread?.session?.status === "starting" ||
          serverThread?.session?.status === "running"
        }
        refreshNativeList={subagentsIntegration.refreshNative}
        maximized={rightPanelMaximized}
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div className="relative flex min-h-0 flex-1 flex-col">
              ...
              <MessagesTimeline
                ...
                contentInsetEndAdjustment={composerOverlayHeight + subagentsSummaryInset}
              ...
              {subagentsIntegration.available ? (
                <div
                  className="pointer-events-none absolute inset-x-0 z-20 px-3 sm:px-5"
                  style={{ bottom: composerOverlayHeight + 8 }}
                >
                  <div className="mx-auto w-full min-w-0 max-w-3xl">
                    <div className="pointer-events-auto w-fit">
                      {subagentsIntegration.renderChatItem(addAgentsSurface)}
                    </div>
                  </div>
                </div>
              ) : null}
              ...
              {showScrollToBottom && (
                <div
                  ...
                  style={{ bottom: composerOverlayHeight + subagentsSummaryInset + 4 }}
                >
                  <button
                    type="button"
